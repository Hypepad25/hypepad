  ...
  return(<header className="header">
    <div className="container nav">
      <div className="nav-left">
        <Link href="/" className="brand"><img src="/logo.svg" alt="HYPEPAD"/></Link>
        <ul className="menu">
          <li><Link href="/leaderboard">Leaderboard</Link></li>
          <li><Link href="/staking">Staking</Link></li>
          {groups.map((g,i)=>(
            <li key={i} className={`dropdown ${openIndex===i?'open':''}`}
                onMouseEnter={()=>setOpenIndex(i)} onMouseLeave={()=>setOpenIndex(-1)}>
              <a href="#" onClick={(e)=>{e.preventDefault(); setOpenIndex(openIndex===i?-1:i);}}>{g.label}<span className="caret"/></a>
      ...
      <div className="nav-actions">
        <button className="btn ghost" onClick={()=>setConnected(!connected)}>{connected?'0x12...abC':'Connect Wallet'}</button>
        <button className="hamburger" aria-label="Menu" onClick={()=>setMobileOpen(!mobileOpen)}>☰</button>
      </div>
    </div>
    <div className={`mobile ${mobileOpen?'open':''}`}>
      <div className="section"><div className="container"><Link href="/leaderboard">Leaderboard</Link><Link href="/staking">Staking</Link></div></div>
      {groups.map((g,i)=>(<div className="section" key={i}><div className="container"><div className="title">{g.label}</div>{g.items.map((it,ii)=>(<Link key={ii} href={it.href}>{it.label}</Link>))}</div></div>))}
    </div>
  </header>);
}
